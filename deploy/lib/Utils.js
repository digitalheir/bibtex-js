...
module.exports['extend'] = function (target, opt_superclass) {
  ...
  if (opt_superclass !== undefined) {
    // if the superclass is defined
...
module.exports['updateProperties'] = function (target, values, opt_keys, opt_attributes) {
  ...
  if (opt_keys === undefined) {
    // if the key map isn't defined
    for (var key in values) {
      // for all the enumerable properties
    ...
    // if the list of the keys is defined
    ...
    // if the map of the keys is defined
    for (var targetKey in opt_keys) {
      // for all the target keys
      //noinspection JSUnfilteredForInLoop
      var _key = opt_keys[targetKey]; // the sources key
      if (values[_key] !== undefined)
        //noinspection JSUnfilteredForInLoop
        Object.defineProperty(target, targetKey, // update the property
        // using the defined value
        Object.create(opt_attributes, { value: { value: values[_key] } }));
    }
  } else {
    // if "keys" has unsupported value
...
module.exports['testProperties'] = function (target, values, opt_keys, opt_skipUndefined) {
  ...
  if (opt_keys === undefined) {
    ...
    for (var key in values) {
      // for all the enumerable properties
      //noinspection JSUnfilteredForInLoop
      if (target[key] !== values[key] && !(values[key] === undefined && opt_skipUndefined)) return false; // false if any value is different
    }
  } else if (opt_keys instanceof Array) {
    // if the list of the keys is defined
    ...
    for (var targetKey in opt_keys) {
      // for all the target keys
      //noinspection JSUnfilteredForInLoop
      var _key2 = opt_keys[targetKey]; // the sources key
      //noinspection JSUnfilteredForInLoop
      if (target[targetKey] !== values[_key2] && !(values[_key2] === undefined && opt_skipUndefined)) return false; // false if any value is different
    }
  } else {
    // if "keys" has unsupported value
    throw new TypeError('"keys" isn\'t an Object instance');
